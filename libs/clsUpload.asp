...
' ------------------------------------------------------------------------------
	...
	End Property	
' ------------------------------------------------------------------------------
	Public Property Get FileExtensionOf(ByRef avKeyIndex)
		Dim lnIndex
		lnIndex = IndexOf(avKeyIndex)
		if lnIndex = -1 Then Exit Property
		FileExtensionOf = oFields(lnIndex).FileExtension
	End Property
' ------------------------------------------------------------------------------
	Public Property Get FileFormatOf(ByRef avKeyIndex)
		Dim lnIndex
		lnIndex = IndexOf(avKeyIndex)
		if lnIndex = -1 Then Exit Property
		FileFormatOf = oFields(lnIndex).FileFormat
	End Property
' ------------------------------------------------------------------------------
...
Public Property Get FileInputName()
	FileInputName = psFileInputName
End Property

' --------------------	----------------------------------------------------------
Public Function Save(binData)
	if psFileFullPath <> "" and psFileInputName <> "" then
		...
		Else
			psError = "[" & Err.Number & "] : " & Err.Description & " - " & Err.Source
		End if
...
' ------------------------------------------------------------------------------
	Private Sub Class_Initialize()
		...
		Dim lsFileExtension     ' File extension
		Dim lsFileFormat        ' File format
		...
		Do Until (lnBoundaryStart = InstrB(lnBytes, lnBoundaryBytes & CStrB("--")))
			...
			If Not lnFileNamePosition = 0 And lnFileNamePosition < lnBoundaryEnd Then
				...
				loField.FileExtension = Right(lsFileName, Len(lsFileName)-InStrRev(lsFileName, "."))
				...
				loField.FileFormat = Right(lsContentType, Len(lsContentType)-InStrRev(lsContentType, "/"))
